...
const Record = ({ id, name, price, quantity, cost, idx, product_id }) => {
  const { showProducts, setShowProduct } = useContext(context);

  const deleteRecord = () => {
    setShowProduct(showProducts.filter((el) => el.id != id));
  };
      ...
      showProducts.map((product) =>
        product.product_id === product_id
          ? {
              ...
              cost: ((product.quantity + 1) * product.price).toFixed(2),
            }
          : product
      )
    );
      ...
      showProducts.map((product) =>
        product.product_id === product_id
          ? {
              ...
              cost: (Math.max(product.quantity - 1, 1) * product.price).toFixed(
                2
              ),
            }
          : product
  ...
  return (
    ...
    >
      <td className="px-6 py-4 td-counter">{idx + 1}</td>
      <th
        scope="row"
        className="record-name capitalize px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white"
      >
        {name}
      </th>
